...
use quote::quote;
use syn::{parse_macro_input, parse_quote, Expr, Ident, ItemFn};
...
pub fn commandeer(args: TokenStream, input: TokenStream) -> TokenStream {
    ...
    let test_file_name = format!("test_{}.json", fn_name);
    ...
    let mock_commands: Vec<Expr> = commands.iter().map(|cmd| {
        parse_quote! {
    ...
    let setup_stmts: Vec<syn::Stmt> = vec![
        parse_quote! {
            let commandeer = crate::Commandeer::new(#test_file_name, crate::Mode::#mode_ident);
        }
    ];
    
    let mock_stmts: Vec<syn::Stmt> = mock_commands.iter().map(|expr| {
        parse_quote! {
            #expr;
        }
    }).collect();
